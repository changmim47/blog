...
import { uploadFile, getPostById } from '../services/storage';
      ...
      if (isEdit && id) {
          ...
          getPostById(id).then(found => {
              if (found) {
                  ...
                  setContentImages(found.contentImages || (found.contentImage ? [found.contentImage] : []));
        ...
        setContentImages(initialPost.contentImages || (initialPost.contentImage ? [initialPost.contentImage] : []));
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in relative">
      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
        <div className="p-8">
            ...
            <div className="flex justify-between items-center pt-6 border-t border-slate-100 gap-4 flex-wrap">
                <label className="flex items-center gap-3 text-sm text-slate-600 cursor-pointer select-none">
                    ...
                    />
                    <span>
                        <span className="font-medium">즉시 발행</span>
                        <span className="text-xs text-slate-400 ml-2">
                            {published ? '체크 해제 시 초안으로 저장됩니다' : '초안으로 저장됩니다 (공개되지 않음)'}
                        </span>
                    </span>
                </label>
                ...
                >
                    {isLoading ? '저장 중...' : (published ? '발행하기' : '초안 저장')}
                </button>
            </div>
        </div>
      </div>
